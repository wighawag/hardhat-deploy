...
function getOldArtifactSync(
  ...
  /**
   * @dev Retreive artifacts for the OVM and EVM
   */
  const oldArtifactPath = path.join(folderPath, name + ('.json' || '-ovm.json'));
  let artifact;
...
// Does adding the ability to retreive both `.json` and `-ovm.json` files in 
